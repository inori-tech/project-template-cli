...
const npm = require("libnpmconfig");

const config = npm.read();
...
function getProcessEnv(env) {
  ...
  if (typeof env === "string") {
    for (const name of [env, `${env}`.toUpperCase(), `${env}`.toLowerCase()]) {
      val = config.get(name) ?? val;
      if (val) {
        break;
      }
    }
...
function getProxy(protocol = "http:") {
  const HTTPS_PROXY_ENV = ["https_proxy"];
  const HTTP_PROXY_ENV = [...HTTPS_PROXY_ENV, "http_proxy", "proxy"];

  let parsed;

  if (protocol.includes("https")) {
    parsed = url.parse(getProcessEnv(HTTPS_PROXY_ENV));
  } else {
    parsed = url.parse(getProcessEnv(HTTP_PROXY_ENV));
  }
...
async function fetchTemplatesFromRemote(repoType = "github", config, targetUrl) {
  try {
    ...
    if (token) {
      axiosConfig.headers["Authorization"] = repoType === "github" ? `token ${token}` : `Bearer ${token}`;
    }

    axiosConfig.proxy = getProxy();
      ...
      const promises = data.map((item) => {
        ...
        return new Promise((resolve, reject) => {
          if (item.download_url && (item.type === "file" || item.type === "blob")) {
            axios
              .get(item.download_url, { proxy: axiosConfig.proxy, responseType: "stream" })
              .then((res) => {
